...
public class NameSurferGraph extends GCanvas implements NameSurferConstants,
	...
	//once you input a name and search, its order in the entry list is fixed, which is entryNumber 
	//entryNumber is used to assign different colors
	private void drawEntry(NameSurferEntry entry, int entryNumber) {
		for (int i = 0; i < NDECADES - 1; i++) {
			//ranking1: starting point ranking2:end point
		...
		for (int i = 0; i < NDECADES; i++) {
			...
			double y = 0;
			if (rank != 0) {
				y = GRAPH_MARGIN_SIZE + (getHeight() - GRAPH_MARGIN_SIZE * 2)
						* rank / MAX_RANK - 5;
			} else {
				label = name + " *";
